...
float rayStrength(vec2 raySource, vec2 rayRefDirection, vec2 coord,
  ...
  float baseStrength = clamp(
    (0.5 + 0.3 * sin(distortedAngle * seedA + iTime * speed)) +
    (0.4 + 0.35 * cos(-distortedAngle * seedB + iTime * speed)),
    0.0, 1.0
  );
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  vec4 rays2 = vec4(1.0) *
               rayStrength(rayPos, finalRayDir, coord, 22.3991, 18.0234,
                           1.1 * raysSpeed);
  vec4 rays3 = vec4(1.0) *
               rayStrength(rayPos, finalRayDir, coord, 15.789, 33.456,
                           0.8 * raysSpeed);

  // Combine rays
  fragColor = rays1 * 0.6 + rays2 * 0.4 + rays3 * 0.3;

  // Add depth-based atmospheric effect
  float depth = length(coord - rayPos) / iResolution.x;
  float atmosphericFade = 1.0 - depth * 0.2;
  ...
  // Basic depth-based lighting
  float brightness = 1.0 - (coord.y / iResolution.y);
  float depthBrightness = 1.0 - depth * 0.1;
  
  // Simple color grading
  fragColor.x *= 0.1 + brightness * 0.4 * depthBrightness;
  fragColor.y *= 0.2 + brightness * 0.5 * depthBrightness;
  fragColor.z *= 0.3 + brightness * 0.6 * depthBrightness;

  // Add subtle depth-based color variation
  fragColor.rgb += vec3(0.05, 0.1, 0.15) * depthBrightness * brightness * 0.2;
